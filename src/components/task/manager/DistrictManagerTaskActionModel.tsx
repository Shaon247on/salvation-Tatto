...
// Using the new District Manager compatible hooks
import { 
  useGetLocationsByDistrictManagerQuery, 
  useGetEmployeesByLocationByDistrictManagerQuery 
} from "@/redux/services/districtManager/distictManagerTaskApi";

interface DistrictManagerTaskActionModelProps {
  isOpen: boolean;
  onClose: () => void;
...
export const DistrictManagerTaskActionModel = ({
  isOpen,
  onClose,
  initialData,
  onSave,
  isLoading = false,
}: DistrictManagerTaskActionModelProps) => {
  const isEditMode = !!initialData;

  // Get token from Redux
  const token = useAppSelector(selectCurrentToken);

  // Fetch locations (District Manager uses the same general location endpoint or specialized one)
  const { data: locationsResponse } = useGetLocationsByDistrictManagerQuery(undefined, {
    skip: !token,
  });
  ...
  // Sync initialData (mapping API fields like 'location_name' to local 'location' state)
  useEffect(() => {
    if (isOpen) {
      setFormData({
        ...
        locationId: initialData?.location || 0,
  ...
  // Fetch employees for selected location using District Manager specific endpoint
